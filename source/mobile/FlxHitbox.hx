...
class FlxHitbox extends FlxSpriteGroup {
  ...
  public var hideHitboxHints:Bool = false;
	...
	public function createhitbox(x:Float = 0, y:Float = 0, width:Int, height:Int, color:Int) {
		...
		if (!hideHitboxHints)
		{
			button.onDown.callback = function()
			{
				...
				hintTween = FlxTween.tween(button, {alpha: controlsAlpha}, controlsAlpha / 100, {
			...
			button.onUp.callback = function()
			{
				...
				hintTween = FlxTween.tween(button, {alpha: 0.00001}, controlsAlpha / 10, {
					ease: FlxEase.circInOut,
					onComplete: function(twn:FlxTween)
					{
						hintTween = null;
	...
	{
		var guh = controlsAlpha;
		if (guh >= 0.9)
			guh = controlsAlpha - 0.07;
